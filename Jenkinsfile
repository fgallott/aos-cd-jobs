...
node {
    ...
    try {
        ...
        stage("Sign RHCOS container images") {
            // Sign all RHCOS container images (kubevirt, oci-manifest, etc.) using sigstore
            // The artcd command will gracefully handle cases where no container images are present
            ...
            withCredentials([
                ...
                try {
                    ...
                    // Don't fail the whole job if RHCOS container signing fails
